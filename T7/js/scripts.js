...
function adicionaNoFicticio(somaOfertas, somaDemandas){
	...
	if (somaOfertas > somaDemandas){
		...
		for (i = 0; i < valorNoFicticio; i++) {

			for (j = 0; j < numOfertas; j++){
				matrizCustos[j][numDemandas+i] = 0;
			}
		}
	} else {
...
function formaPadrao(){
	...
	somaOfertas = numOfertas;
	somaDemandas = numDemandas;
...
function imprimeFP(somaOfertas, somaDemandas){
	...
	if (somaOfertas === somaDemandas){
		...
		$div_msg.append('&sum;O > &sum;D. Adicionado '+valorNoFicticio+' n&oacute;(s) fict&iacute;cio(s) de demanda.');
	} else {
		$div_msg.append('&sum;O < &sum;D. Adicionado '+valorNoFicticio+' n&oacute;(s) fict&iacute;cio(s) de oferta.');
	}
}
